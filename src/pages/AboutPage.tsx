...
export function AboutPage() {
  return (
    <>
      <PageHeader
        eyebrow="Company"
        title="The standard, set in full"
          ...
          <Reveal className="flex flex-col gap-8">
            <div className="flex flex-col gap-4">
              ...
              <h2 className="display text-4xl sm:text-5xl">
                A higher bar for systems installation
              </h2>
            </div>
            <div className="flex flex-col gap-4 text-base leading-relaxed text-muted-foreground">
              <p>
                PI Locks was founded to set a higher bar for systems installation —
                a team of experienced integrators who take full accountability for
                design, installation, commissioning, and support, delivered to a
                premium standard on every project.
              </p>

              {/* The "PI" promise — what "premium" actually means here. */}
              <blockquote className="rounded-2xl border-l-2 border-brass bg-card p-6">
                <p className="text-sm leading-relaxed text-foreground/90">
                  “What does premium mean here? The system arrives fully designed,
                  installed, tested, documented, and commissioned — ready to use on
                  day one, and backed by responsive service long after handover.
                  No half-finished installations. No loose ends. That’s the PI
                  Locks standard.”
                </p>
              </blockquote>
            </div>
      ...
      <section className="bg-blueprint border-y border-border/60 py-20 sm:py-28" aria-label="The PI Standard">
        <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
          <Reveal>
            <SectionHeading
              eyebrow="Our Standard"
              title="Auditable proof, not statistics"
          ...
          <div className="mt-14 grid gap-6 sm:grid-cols-2">
            {standard.map((item, i) => (
              <Reveal
                key={item.title}
                delay={(i % 2) * 100}
                className="flex flex-col gap-3 rounded-2xl border border-border/60 bg-card p-7 transition-colors duration-300 hover:bg-steel-2"
              >
      ...
      <section className="mx-auto w-full max-w-7xl px-4 py-20 sm:px-6 sm:py-28 lg:px-8" aria-label="Team">
        <Reveal>
          <SectionHeading
            eyebrow="The team"
            title="Roles we hold the line with"
            lede="Every engagement is owned end to end — one accountable set of names, from first site visit to years later."
          />
        </Reveal>
        <div className="mt-14 grid gap-px overflow-hidden rounded-2xl border border-border/60 bg-border/60 sm:grid-cols-3">
          {team.map((member, i) => (
            <Reveal
              key={member.role}
              delay={i * 100}
            ...
            >
              <span className="display text-xl text-brass">{member.role}</span>
              <p className="text-sm leading-relaxed text-muted-foreground">
                {member.body}
              </p>
            </Reveal>
      ...
      <section className="border-t border-border/60 py-16" aria-label="Work with us">
        <div className="mx-auto flex w-full max-w-7xl flex-col items-start gap-6 px-4 sm:px-6 lg:flex-row lg:items-center lg:justify-between lg:px-8">
          <div className="flex max-w-2xl flex-col gap-3">
            <h2 className="display text-4xl sm:text-5xl">
              Work with a partner who owns the outcome
            </h2>
